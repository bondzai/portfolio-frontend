...
interface DisplayModalProps {
    ...
    dataRoutePath: string;
}

const DisplayModal: React.FC<DisplayModalProps> = ({ getDataList, dataRoutePath }) => {
    const { id } = useParams();
    const navigate = useNavigate();
    const [current, setCurrent] = useState<number>(Number(id));
    const [dataList, setDataList] = useState<DataItem[]>([]);
    const [dataListFetched, setDataListFetched] = useState<boolean>(false);
